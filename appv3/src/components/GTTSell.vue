...
<!-- <button @click = "deleteGTT( 1 )">Delete GTT</button> -->
    ...
    <v-chip>Total {{ stocksPricePointsFiltered.length }} </v-chip>   
     ...
     @click = " getStockPricePoints(  )">GET Holdings </button>
...
<span v-if = "typeof stockPpItem.gttData!== 'undefined' ">
            ...
            <td class = "text-left">
                   ...
                   <button class="my-btn"
                   @click = "PlaceGttSELLOrderForThisPrice( stockPpItem,l.level )"
                   >{{ l.level }} 
...
import PlaceGttSELLOrderForThisPrice  from './PlaceGttSELLOrderForThisPrice';
    ...
    this.$set( 
       e
        ...
        'NO_GTT_PRESENT'
             ...
             PlaceGttSELLOrderForThisPrice( stockPpItem,level){

PlaceGttSELLOrderForThisPrice(stockPpItem,level,this.gttAmountPerManualOrder,this.accessToken)
