<template>
 ...
 <v-btn @click="zerodhaPositions = !zerodhaPositions"> Live position </v-btn>
   

    <div class="row">
      <div class="col">
<h1>from Zerodha</h1>
<table class="table" v-if="zerodhaPositions">
      <tr
        v-for="(livepos, index) in livePositionsFiltered"
        :key="livepos.instrument_tocken"
      >
        <td>{{ index + 1 }}</td>

        <td>{{ livepos.tradingsymbol }}</td>

        <td>{{ livepos.product }}</td>
        <td>{{ livepos.quantity }}</td>
        <td>{{ livepos.average_price }}</td>
        <td>{{ livepos.last_price }}</td>
        <td
          :class="{
            'text-success': livepos.pnl > 0,
            'text-danger': livepos.pnl < 0,
          }"
        >
          {{ livepos.pnl }}
        </td>
...
<table class="table" v-if="zerodhaPositions">
      <tr
        v-for="(livepos, index) in localStorage"
        :key="livepos.instrument_tocken"
      >
        <td>{{ index + 1 }}</td>
    ...
    <v-btn v-if="!showSignals" @click="showSignals = true">
      show Signals
    </v-btn>
    <v-btn v-if="showSignals" @click="showSignals = false">
      Hide signals
    </v-btn>
    ...
    <v-col cols="5" offset="4">
      <v-alert
        ...
        type="info"
      >
        Long buy above tc target r1 sl bc & short below bc target s2 sl
        tc</v-alert
      >
    </v-col>
    ...
    <v-row align="center" class="mb-2">
      ...
      <v-col cols="3">
        ...
        <input
          type="text"
          class="form-control"
          v-model="amountPerStock"
          placeholder="Amount Per Stock"
        />
      </v-col>
      ...
      <v-col cols="1">
        <v-btn
          ...
          icon
          title="Refresh"
          @click="fetchSymbolsForTheCategory"
        >
      ...
      <v-col cols="1">
        <v-btn
          color="red"
          icon
          title="Refresh trade status"
          @click="refreshTradeStatus"
        >
          <v-icon>mdi-refresh</v-icon>
        </v-btn>
      </v-col>
...
<v-row style="height:3000vh;overflow-y:auto">
  <v-col>
...
<table class="table table-success table-bordered">
  ...
  <tbody>
    <tr v-for="script in scripts" :key="script.symbol" 
    
    :class="{'noTradable':!script.longTrade}"
    >
      <td>{{ script.symbol }}
      ...
      <td>{{ script.ltP }}</td>
      <td>{{ JSON.parse(script.pricePoints.pivotPoints).tc }}</td>
      <td>{{ JSON.parse(script.pricePoints.pivotPoints).r1 }}



        <small class="pull-right pull-bottom">
  Profit 	 {{Math.round(JSON.parse(script.pricePoints.pivotPoints).r1-JSON.parse(script.pricePoints.pivotPoints).tc)}}&#x20B9; 

  RskRwd {{script.LongRiskReward}}
</small>

      </td>
     
     
      <td>{{ JSON.parse(script.pricePoints.pivotPoints).bc }}

<small class="pull-right pull-bottom">
  Loss 	 {{Math.round(JSON.parse(script.pricePoints.pivotPoints).tc-JSON.parse(script.pricePoints.pivotPoints).bc)}}&#x20B9; 
</small>
...
<table class="table table-danger table-bordered">
  ...
  <tbody>
    <tr v-for="script in scripts" :key="script.symbol" 
    :class="{'noTradable':!script.shortTrade}"
    >
      <td>{{ script.symbol }}</td>
      <td>{{ script.ltP }}</td>
      <td>{{ JSON.parse(script.pricePoints.pivotPoints).bc }}</td>
      <td>{{ JSON.parse(script.pricePoints.pivotPoints).s1 }}

       <small class="pull-right pull-bottom">
  Profit 	 {{Math.round(JSON.parse(script.pricePoints.pivotPoints).bc-
  JSON.parse(script.pricePoints.pivotPoints).s1)}}&#x20B9; 

  RskRwd {{script.shortRiskReward}}
</small>



      </td>
      <td>{{ JSON.parse(script.pricePoints.pivotPoints).tc }}
...
export default {
  ...
  computed: {
    livePositionsFiltered() {
      return this.livePositions.filter((lp) => lp.quantity != 0 && lp.product=='MIS');
    },
      ...
      let openTrades = trades
        ...
        .forEach((t) => {
          ...
          if (this.LatestQuotes.length > 0) {
            last_price = this.LatestQuotes.filter(
              (lq) => lq.tradingsymbol == t.symbol
            )[0].last_price;
...
/*
      ...
      tmp.forEach((t) => {
        let trade = this.livePositions.filter(
          (lp) => lp.tradingsymbol == t.symbol
        )[0];
        ...
        if (trade.quantity != 0) {
          ...
          if(t.status!='CYCLE COMPLETED'){
t.status = "COMPLETE";
          t.quantity = Maths.abs(trade.quantity);
    ...
    checkTradingScriptOrderCompleted(symbol) {
      ...
      let ln = trades.filter(
        (t) => t.symbol == symbol && t.status == "COMPLETE"
      ).length;

      // console.log(
      //   `from inside checkTradingScriptOrderCompleted for symbol ${symbol} and boolean for ln ${ln}`
      // );
      if (ln > 0) {
        return true;
      } else {
    ...
    checkTradingScriptOrderCycleCompleted(symbol) {
      ...
      let ln = trades.filter(
        (t) => t.symbol == symbol && t.status == "CYCLE FINISHED"
      ).length;

      // console.log(
      //   `inside from checkTradingScriptOrderCycleCompleted function for symbol ${symbol} and ln to find boolean is ${ln} `
      // );
      if (ln > 0) {
        return true;
      } else {
      ...
      skt.forEach((symbol) => {
        ...
        if (!hasTraded) {
          ...
          if (typeof chkPricePoints == "undefined") {
            console.log(
              "PRICE POINT NOT SET FOR THE SCRIPT ",
         ...
         // < r1 && symbol.pvs_ltP<=tc
          if (symbol.last_price > tc && symbol.last_price ) {

// if profit is not +ve do not trade
let profitLong=r1-tc;

let lossLong=tc-bc;

let LongRiskReward=profitLong/lossLong;

if(lossLong<2){

  console.log('No trade since stop loss is too close')
  return false
}

if(profitLong<=0){
console.log('No trade since profit is not favorable')
  return false
}

if(LongRiskReward<2){
console.log('No trade since Risk reward is not  favorable')
  return false
...
// if profit is not +ve do not trade
let profitShort=bc-s1;

let lossShort=tc-bc;

let shortRiskReward=profitShort/lossShort;
...
console.log('No trade since Risk reward is not  favorable')
  return false
        ...
        let hasTradingScriptOrderCompleted =
          this.checkTradingScriptOrderCompleted(CurScript.symbol);

        let hasTradingScriptOrderCycleCompleted =
          this.checkTradingScriptOrderCycleCompleted(CurScript.symbol);

        //let hasReverseOrderPlaced=this. hasReverseOrderPlaced(CurScript.symbol)

        if (
          hasTraded &&
          hasTradingScriptOrderCompleted &&
          !hasTradingScriptOrderCycleCompleted
        ) {
          ...
          // console.log(
          //   "this.getScriptTradedStatus(CurScript.symbol)",
          //   this.getScriptTradedStatus(CurScript.symbol)[0]
          // );
            ...
            switch (transaction_type) {
              case "BUY":
                if (symbol.last_price >= r1) {
                 ...
                 // console.log("placed target order for ", tradingsymbol);
                  ...
                  var txt = `Target hit for symbol ${tradingsymbol} , Squareoff  @ ${
                    obj.price
                  }. Advise to ${
                    CurTraded[0].transaction_type
                  } given @ Price ${tc} .Profit Per stock is ${obj.price - tc}`;
                  this.sendToTelegramGroup(txt);
                }
                ...
                if (symbol.last_price <= bc) {
                  ...
                  var txt = `Stop Loss hit for symbol ${tradingsymbol} , Squareoff  @ ${
                    obj.price
                  }. Advise to ${
                    CurTraded[0].transaction_type
              ...
              case "SELL":
                if (symbol.last_price >= s1) {
                  ...
                  // console.log(
                  //   "placed target order short covering ",
                  //   tradingsymbol
                  // );
                  ...
                  var txt = `Target hit for symbol ${tradingsymbol} , Squareoff  @ ${
                    ...
                    CurTraded[0].transaction_type
                  } given @ Price ${bc} .Profit Per stock is ${obj.price - bc}`;
                ...
                if (symbol.last_price >= tc) {
                  ...
                  // console.log(
                  //   "placed stop losss order short covering ",
                  //   tradingsymbol
                  // );
                  ...
                  var txt = `Stop Loss hit for symbol ${tradingsymbol} , Squareoff  @ ${
                    ...
                    CurTraded[0].transaction_type
                  } given @ Price ${bc} .Loss Per stock  is ${obj.price - tc}`;
   ...
   let pivotPoints = JSON.parse(pp.pivotPoints);
...
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"pricePoints",pp);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"bc",bc);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"tc",tc);


this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"s1",s1);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"s2",s2);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"s3",s3);


this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"r1",r1);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"r2",r2);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"r3",r3);
...
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"profitLong",profitLong);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"lossLong",lossLong);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"LongRiskReward",LongRiskReward);
...
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"profitShort",profitShort);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"lossShort",lossShort);
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"shortRiskReward",shortRiskReward);



this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"pivotPoints",pivotPoints);


let longTrade=false;

if(profitLong<=0 || LongRiskReward<2 || lossLong<2){
  longTrade=false;
console.log('No trade since profit is not favorable')
  return false
}else{
...
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"longTrade",longTrade);


let shortTrade=false;
if(profitShort<=0 || shortRiskReward<2 || lossShort<2){
  shortTrade=false
console.log('No trade since profit is not favorable')
  return false
}else{
...
this.$set(this.scripts.filter((s) => s.instrument_token == CurrentScript.instrument_token)[0],"shortTrade",shortTrade);
    ...
    mutateWithPricePointNormal() {
      ...
      let ti = setInterval(async () => {
        ...
        this.$set(
          this.scripts.filter(
            (s) => s.instrument_token == s.CurScript.instrument_token
          )[0],
          "pricePoints",
          pp
        );
  ...
  mounted() {
    setInterval(() => {
      ...
      //this.zerodhaPositions = !this.zerodhaPositions;
    }, 3000);
    ...
    window.setInterval(() => {
      ...
      if (this.hours == 15 && this.minutes < 10 && this.seconds < 0) {
        //cancel all orders
    ...
    socket.on("order_update", (orderUpdates) => {
      ...
      let sameOrderUpdate =
        Object.entries(this.previousOrderUpdate).toString() ===
        Object.entries(orderUpdates).toString();
      ...
      let CurrentCycleStatus = this.getScriptTradedStatus(tradingsymbol).filter(
        (t) => t.status == "CYCLE FINISHED"
      ).length;
      ...
      if (CurrentCycleStatus > 0) {
        console.log(
          "cycle finished status for CurrentCycleStatus ",
          CurrentCycleStatus,
          tradingsymbol
        );
