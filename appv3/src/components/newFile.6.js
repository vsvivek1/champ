...
import { hourlyPricePointsofLiveDay1, socket, instruments, comF, shareF } from './NewFutureMining.vue';
...
if (this.itype == "MCX") {
this.setter = comF;
let instrumentsForMining1 = await this.requireJson("../../../instruments/" + this.setter);
...
const {
tradable, mode, last_price, last_traded_quantity, average_traded_price, volume_traded, total_buy_quantity, total_sell_quantity, ohlc: { open, high, low, close }, change, last_trade_time, exchange_timestamp, oi, oi_day_high, oi_day_low,
...
return arr.prices.sort((a, b) => a.date < b.date ? -1 : (a.date > b.date ? 1 : 0)[0]
);
},
...
if (this.hourlyPricePointsofLiveDay.filter(
(i) => i.instrument_token == instrument_token
)[0].prices.length > 0) {
...
// o.variety = i.variety;
// o.variety = 'regular';
if ((this.hours == 15 && this.minutes > 30) ||
this.hours > 15 ||
this.hours < 9 ||
...
proposedStopLoss = cis.last_price * 0.9;

if (i.price < proposedStopLoss * 0.95) {
...
proposedSopLossPrice = cis.last_price * 0.9;
o.variety = 'regular';
o.order_id = i.order_id;
let params = {};
params.price = proposedSopLossPrice.toFixed(1);
...
if (typeof hourlyPricePointsofLiveDay.filter(
(i) => i.instrument_token == instrument_token
)[0] == "undefined") {
return -111;
}
...
if (hourlyPricePointsofLiveDay.filter(
(i) => i.instrument_token == instrument_token
)[0].prices.length > 0) {
...
// return instrument_token;
if (typeof this.instruments.filter(
(i) => i.instrument_token == instrument_token
)[0] == "undefined") {
// return 1000;
}
...
if (typeof this.livePositions.filter(
(lp) => lp.instrument_token == instrument_tocken
)[0] != "undefined") {
return (
...
this.livePositionsSelected.forEach((lp) => this.$set(lp, "selected", true)
);
...
let result = o.exchange == this.itype &&
(o.status == "OPEN" ||
o.status == "AMO" ||
...
element.ohlc.low <= element.last_price * 0.5) {
...
// console.log( element.last_price == element.ohlc.high,ts )
if (element.last_price < element.ohlc.high * 1.01 && element.last_price > element.ohlc.high * 0.99 && !cis.tradingsymbol.includes('FUT')) {
...
(element.ohlc.high - element.ohlc.open) / element.ohlc.open < 0.2 &&
...
.filter((o) => o.tradingsymbol == l.tradingsymbol)[0].price;
...
let count, rangeBreakOut, high, targetPoint;


let { upperBreakOutTarget, lowerBreakOutTarget } = this.getLatestPricePoints(instrument_token);
...
if ((this.hours == 23 && this.minutes >= 30) ||
this.hours == 0 ||
this.hours < 9 ||
(this.hours == 9 && this.minutes < 15)) {
order.variety = "amo";
...
/// fire a target
let count, transaction_type, rangeBreakOut, high, targetPoint, PlacedReverseOrder;
...
let trade = `Buy instrument ${cis.tradingsymbol}  at ${cis.pricePoints.yesterday.high}  . 
      Target ${cis.pricePoints.yesterday.rangeBreakOutTarget}  
      Strict stop loss at ${Math.max(
cis.pricePoints.pivotPointObject.bc.toFixed(1),
cis.pricePoints.d1.low
)}  ,
      TargetProfit ${(cis.pricePoints.yesterday.rangeBreakOutTarget -
cis.pricePoints.yesterday.high) *
cis.lot_size} 
      Possible Loss ${(cis.pricePoints.yesterday.high -
cis.pricePoints.pivotPointObject.bc.toFixed(1)) *
cis.lot_size} 
      `;
...
let todayOpenYesterDayhigh = element.ohlc.open > cis.pricePoints.d1.high && element.last_price < (Math.round(cis.pricePoints.d1.high * 0.95, 1));
let todayOpenYesterDayClose = element.ohlc.open > cis.pricePoints.d1.close && element.last_price < (Math.round(cis.pricePoints.d1.close * 0.95, 1));
...
// let squareoffDuringSideWise = ( sideWisetime && livePnlOffered>0 );
let buyPriceAboveOpenAndLastPriceFallsBelowOpen = (positionObj.buy_price > element.ohlc.open && element.last_price < element.ohlc.open * 0.95);
...
case (lp.quantity > 0 && element.last_price < element.ohlc.open * 0.98 && false):
