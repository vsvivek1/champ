<template>
  <div>
    ...
    <div v-if = "hourlyPricePointsofLiveDay.length">
      <!-- {{ 
       sortArrayByDateDesc( hourlyPricePointsofLiveDay[1] )
    ...
    <!-- {{ livePositions[0] }} livePositions. -->

    <!-- this.{{ closedTradesScripts }}  -->
    <!-- {{ tradingAlerts }} tradingAlerts -->

    <!-- {{ instruments.filter( i =>i.tradingsymbol == 'ABBOTINDIA22JUN17000PE' )[0] }}  -->

    <!-- {{ livePositions[1].instrument.pricePoints }} livePositionsSelected -->
...
export default { 
  ...
  mounted(  ) { 
    ...
    let thirtyMinuteTimer  =  setInterval((  )  => { 
      ...
      // var  hourlyPricePointsofLiveDay  =  require( "../../../instruments/hourlyCandleData.json" );
    ...
    //  } 
    this.instruments  =  instrumentsForMining.filter( i =>i.instrument_type!= "FUT" );
...
try{ 
  ...
  methods: { 
    ...
    sortArrayByDateDesc( arr ){ 
      ...
      return arr.prices.sort(( a, b ) => ( a.date< b.date ) ? -1 : 
      (( a.date > b.date ) ? 1 : 0 )[0] )

     } ,
    async cancelLiveOrders(  ) {  } ,

    async updateSellorderWithDesiredPrice( 
      CurrentInstrument,
      element,
      squaringOff,
      p  =  0
      ...
      let CurrentOrderObj  =  this.orders.filter( 
        ( o )  =>
          o.transaction_type  ==  "SELL" && o.instrument_token  ==  instrument_token
       );
      ...
      console.log( 
        CurrentInstrument.tradingsymbol,
        "has hit stop loss inside hasLiveTarget new order is ",
        this.newOrder
       );
    ...
    updateMissingScriptInInstrumetsFile( instrument_token ) { 
      ...
      axios.post( url, params ).then(( r )  => { 
        // let instruments1  =  require( "../../../instruments/instrumentsForMining.json" );


let instruments = instruments1.filter( i =>i.instrument_type!= "FUT" );
    ...
    async showModalForSquareOff(  ) { 
      ...
      this.livePositionsSelected  =  livePositionsTmp.net.filter( 
        ( p )  => p.exchange  ==  "NFO" && p.quantity !=  0 && p.product  ==  "NRML"
       );
    ...
    async getOrders(  ) { 
      ...
      return axios.post( url, obj ).then(( res )  => { 
        ...
        this.executedOrders  =  res.data.filter( 
          ( r )  => r.transaction_type  ==  "BUY" && r.status  ==  "COMPLETE"
         );
        this.allOrders  =  res.data;
        let t  =  res.data.filter(( o )  => { 
          // console.log( o.status,'o.status' );
          let result  = 
            o.exchange  ==  "NFO" && ! o.tradingsymbol.includes( "FUT" ) &&
            ( o.status  ==  "OPEN" ||
              o.status  ==  "AMO" ||
        ...
        this.liveOrders  =  t.filter(( tx )  => tx.exchange  ==  "NFO" && ! tx.tradingsymbol.includes( "FUT" ));
        this.orders  =  t.filter(( tx )  => tx.exchange  ==  "NFO" && ! tx.tradingsymbol.includes( "FUT" ));
      ...
      return axios.post( url, obj ).then(( res )  => { 
        ...
        if ( res.data.net.length > 0 ) { 
          this.livePositionScripts  =  res.data.net
            .filter(( n )  => n.exchange  ==  "NFO" && n.quantity !=  0 )
            .map(( i )  => { 
              if ( 
                this.instruments.filter( 
      ...
      return new Promise(( res, rej )  => { 
        ...
        let timer  =  setInterval((  )  => { 
          ...
          if ( typeof e  ==  "undefined" ) { 
            ...
            console.log( 
              "clearing target timer .. target is finishedd for %s number of scripts"
             );
            clearInterval( timer );
            return;
           } 
        ...
        if ( typeof livePositionsTmp.net !=  "undefined" )
          if ( livePositionsTmp.net.length  ==  0 ) { 
            ...
            tmp  =  livePositionsTmp.net
              .filter( 
                ( p )  =>
                  p.exchange  ==  "NFO" && p.quantity !=  0 && p.product  ==  "NRML"
    ...
    otherCriteria( s, CurrentInstrument,entryPoint ) { 
      try { 
        let element  =  s;
        ...
        // if ( s.ohlc.high >=  entryPoint ) { 
         if ( s.ohlc.high >=  CurrentInstrument.pricePoints.d1.high ) { 
          let ob  =  {  } ;
          ob.otherCriteriaCheck  =  false;
          ob.message  =  "already croseed the yesterday high once  point  once and so avoiding";
            
           return ob;          
         } 


        if ( s.ohlc.open >=  entryPoint ) { 
          let ob  =  {  } ;
          ob.otherCriteriaCheck  =  false;
          ob.message  =  "Price Opened above entry point avoiding .....";
    ...
    mutateWithLtp( s ) { 
      ...
      s.forEach(( element )  => { 
        ...
        this.getHourlyEntryPointsOfDay( instrument_token  )
        ...
        try { 
          ...
          if ( yesterdayCandleColor  ==  "green" ) { 
            ...
            CurrentInstrument.otherCriteria.upperShadow>
            CurrentInstrument.otherCriteria.body
              ...
              entryPoint  =  CurrentInstrument.pricePoints.d1.high;
        ...
        if ( ln > 0 ) { 
          ...
          let offerPrice  =  element.depth.buy.sort( 
            ( a, b )  => b.price - a.price
           )[0].price;
          ...
          if ( PlacedReverseOrder  ==  true || hasLiveTarget  ==  true ) { 
            ...
            if ( hasLiveTarget  ==  true ) { 
              //double checking whther ther is a live sell order
              ...
              let executionTime;
              if ( 
                typeof this.executedBuyOrdersTime.filter( 
                  ( i )  =>
                    i.instrument_token  ==  CurrentInstrument.instrument_token
                 )[0]  ==  "undefined"
               ) { 
                executionTime  =  moment(  ).format( "Y-MM-DD 09:15:00" );
               }  else { 
                executionTime  =  this.executedBuyOrdersTime.filter( 
                  ( i )  =>
                    i.instrument_token  ==  CurrentInstrument.instrument_token
                 )[0].exchange_update_timestamp;
               } 
              ...
              if ( 
                !this.holdingScripts.includes( CurrentInstrument.tradingsymbol )
               ) { 
              ...
              // console.log( 
              //   "live pnl is %s and offred pnl is %s for %s ",
              //   livePnl,
              //   livePnlOffered,
              //   CurrentInstrument.tradingsymbol
              //  );
              ...
              switch ( true ) { 
                ...
                case this.squareOffNow:

                console.log( 
                    livePnl,
                    "sqURING OFF NOW FORCED ",
                    CurrentInstrument.tradingsymbol
                   );

                this.updateSellorderWithDesiredPrice( 
                    CurrentInstrument,
                    element,
                    false,
                    offerPrice
                ...
                case livePnl >=  5000:
                  console.log( 
                    livePnl,
                    "livePnl trigger for 1000 profit ",
                    CurrentInstrument.tradingsymbol
                   );


                  console.log( 'profit above 5 k' );
                  this.updateSellorderWithDesiredPrice( 
                    CurrentInstrument,
                    element,
                    false,
                ...
                case livePnl > 2000 &&
                  (( this.hours  ==  14 && this.minutes > 30 ) || this.hours > 15 ):
                  console.log( 
                    livePnl,
                    "livePnl trigger for 1000 profit ",
                    CurrentInstrument.tradingsymbol
...
return ;
                ...
                case ( this.hours  ==  15 &&
                  ...
                  console.log( 
                    CurrentInstrument.tradingsymbol,
                    "updating to latest price 15 hrs"
                   );
...
return ;
                ...
                case CurrentInstrument.last_price <= 
                  CurrentInstrument.pricePoints.d1.low - 0.05 * 3:
                  // case ( CurrentInstrument.last_price<= ( element.ohlc.low-.05*4 )):

                  /// los bewlow todays low minus 4 ticks


                  let p = Math.min( CurrentInstrument.pricePoints.d1.open,
                  CurrentInstrument.pricePoints.d1.close )
                  this.updateSellorderWithDesiredPrice( 
                    CurrentInstrument,
                    element,
                    false,
