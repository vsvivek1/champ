...
import { writeFinalScriptsTofile } from './writeFinalScriptsTofile.mjs'; // Assuming it's an ES module

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

const dotenvPath = resolve(__dirname, '../../findiserver/.env');

await import('dotenv').then(({ default: dotenv }) => {
    dotenv.config({ path: dotenvPath });
});

let AccesTocken = require('../models/AccessTokens.mjs'); // Assuming it's an ES module
...
//     getStockPricePointsLoader} from '../appv3/src/components/gttComponent.mjs'; 
// 
class Gtt {
