...
const Fetch  =  require( './fetch.js' )  // to download insgruments from zerodha whole
...
async function fetchInstrumentsForMining( accessToken ) { try { 
	 ...
	 console.log( 'file ${fileInputName} deleted successfully ' );
	 }  )
	  ...
	  let f  =  await new Promise(( res, rej )  => { 
	    ...
	    Fs.writeFile( fileOutputName, JSON.stringify( jsonObj1 ), 'utf8',
	      function ( err ) { 
	        ...
	        let targetDir  =  Path.join( FILE_LOCATION+'/nseInstruments.json' );
	
	
	        return Fs.copyFile( FILE_LOCATION+'/nseInstruments.json', targetDir,
