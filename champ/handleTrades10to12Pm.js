...
export function handle10to12PM(cis, kite) {
    ...
    if (breakoutOccurred) {
        proceedToTrade = true;
        if (global.seconds == 30) console.log('15-minute breakout occurred in 10-12 PM', cis.tradingsymbol);
    }
    ...
    if (cis.tick.last_price > h && cis.minuteData.slice(-1)[0].high < h) {
        proceedToTrade = true;
        if (global.seconds == 30) console.log('Price above hourly high and below last candle high', cis.tradingsymbol);
    }
    ...
    if (isHammerCandle(cis.minuteData.slice(-1)) && cis.tick.last_price > cis.minuteData.slice(-1)[0].high) {
        proceedToTrade = true;
        if (global.seconds == 30) console.log('Hammer candle pattern detected and price above last candle high', cis.tradingsymbol);
    }
